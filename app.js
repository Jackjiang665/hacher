...
function showModal(html,wide=false){const wrap=$('#modalWrap');if(!wrap.classList.contains('show'))modalReturnFocus=document.activeElement;$('#modalContent').innerHTML=html;$('#modal').classList.toggle('project-detail-modal',wide);wrap.classList.add('show');wrap.setAttribute('aria-hidden','false');$('#overlay').classList.add('show');$('#overlay').setAttribute('aria-hidden','false');syncOverlayState();requestAnimationFrame(()=>{const field=$('#modalContent [autofocus],#modalContent input:not([type="hidden"]):not(:disabled),#modalContent textarea:not(:disabled),#modalContent select:not(:disabled)');(field||$('#modal')).focus({preventScroll:true})})}
