...
function renderProjectWorkspace(){
  ...
  if(activeProjectTab==='overview'){
    ...
    const files=projectWorkspaceFiles(project);content.innerHTML=`<article class="project-workspace-card"><div class="project-card-head project-file-toolbar"><div><p>项目资料</p><h2>文件与成果</h2></div><select id="projectFileCategory">${Object.entries(projectFileCategoryLabels).map(([value,label])=>`<option value="${value}">${label}</option>`).join('')}</select><button class="outline-btn" data-action="project-link-files">关联原文件</button><button class="solid-btn" data-action="project-import-files">复制进资料库</button></div><div class="project-file-list workspace-list">${files||projectWorkspaceEmpty('还没有项目资料','可以保存独立副本，也可以关联电脑上的原文件。')}</div></article>`;
